...
import (
	...
	"github.com/zcheng9/FlyclientDemo/crypto/secp256k1"
	"github.com/zcheng9/FlyclientDemo/log"
	"math/big"
)
...
func Evaluate(sk *ecdsa.PrivateKey, seed []byte) (index [32]byte, proof []byte) {
	...
	// Index = H(vrf)
	//log.LogPrint("Evaluate","the vrf is:","vrf",hexutil.Encode(vrf))
	index = sha256.Sum256(vrf)
	...
	/*	log.LogPrint("Evaluate","the s len is:","len",len(s.Bytes()))
		log.LogPrint("Evaluate","the s is:","s",hexutil.Encode(s.Bytes()))
		log.LogPrint("Evaluate","the t len is:","len",len(t.Bytes()))
		log.LogPrint("Evaluate","the t is:","t",hexutil.Encode(t.Bytes()))
		log.LogPrint("Evaluate","the buf is:","buf",hexutil.Encode(buf.Bytes()))*/
