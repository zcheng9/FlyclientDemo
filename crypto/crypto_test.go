...
func TestValidSigValues(t *testing.T) {
	check := func(expected bool, r, s, v *big.Int) {
		if ValidSigValue(r, s, v) != expected {
			t.Errorf("mismatch for r: %d s: %d v: %d want: %v", r, s, v, expected)
		}
	}
	...
	// correct v,r,s
	check(true, one, one, zero)
	check(true, one, one, one)
	// incorrect v, correct r,s,
	check(false, one, one, two)

	// incorrect v, combinations of incorrect/correct r,s at lower limit
	check(false, zero, zero, two)
	check(false, zero, one, two)
	check(false, one, zero, two)
	check(false, one, one, two)

	// correct v for any combination of incorrect r,s
	check(false, zero, zero, zero)
	check(false, zero, one, zero)
	check(false, one, zero, zero)

	check(false, zero, zero, one)
	check(false, zero, one, one)
	check(false, one, zero, one)

	// correct sig with max r ,s exceed upper bound
	check(false, secp256k1nMinus1, secp256k1nMinus1, zero)
	// correct v, combinations of incorrect r,s at upper limit
	check(false, secp256k1N, secp256k1nMinus1, zero)
	check(false, secp256k1nMinus1, secp256k1N, zero)
	check(false, secp256k1N, secp256k1N, zero)

	// current callers ensures r,s cannot be negative, but let's test for that too
	// as crypto package could be used stand-alone
	check(false, minusOne, one, zero)
	check(false, one, minusOne, zero)
}
